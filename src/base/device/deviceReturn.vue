      ...
      <div class="form-container">
          <el-form label-width="110px" size="small">
            <el-form-item label="所属课程：">
              <el-select v-model="returnForm.courseId" placeholder="请选择课程" class="diaInput">
                <el-option v-for="item in courseList" :key="item.id" :label="item.name" :value="item.id" style="width:200px"></el-option>
              </el-select>
            </el-form-item>
            <el-form-item label="设备号：">
               <el-input v-model="returnForm.id" placeholder="请输入设备号" class="diaInput" style="width:200px"></el-input>
            </el-form-item>
            <el-form-item label="是否损坏：">
              <el-switch
                v-model="returnForm.isBad"
                active-color="#ff4949"
                inactive-color="#13ce66">
              </el-switch>  
            </el-form-item>
            <el-form-item label="操作：">
              <el-button size="small" type="myPrimary" @click="borrow()">归还</el-button>  
            </el-form-item>
          </el-form>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    borrow(){
      if(this.returnForm.id == ''){
        this.$message.error("设备号不能为空")
      }else if(this.returnForm.courseId == ''){
        this.$message.error("课程不能为空")
      }else{
        myPost('/device/return',this.returnForm) 
        .then(res => {
          this.$message({
            type: "success",
            message: "归还成功"
